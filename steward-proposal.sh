...
PROPOSAL=$SCRIPT_DIR/utils/steward_proposal.json
echo $PROPOSAL
...
sed -i -e "s/\"address\":.*/\"address\": \"$ACCOUNT_ADDRESS\"/g" $PROPOSAL

# Change the add field of the steward
sed -i -e "s/\"add\":.*/\"add\": \"$ACCOUNT_ADDRESS\",/g" $PROPOSAL

echo "New proposal is catted below:"
cat $PROPOSAL

$NAMADA_BIN_DIR/namadac --base-dir "$BASE_DIR_2" init-proposal --pgf-stewards --data-path $SCRIPT_DIR/utils/steward_proposal.json --signing-keys $SIGNING_KEYS --node $NODE
